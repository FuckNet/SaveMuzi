...
public class Player {
	...
	private static final int PWIDTH = 80, PHEIGHT = 80;
	...
	public void paint(Graphics g) {
		...
		if (shieldPoint > 2) {
			...
			// drawImageAnc(shield,
			// (int)(Math.sin(Math.toRadians((smThread.cnt%72)*5))*16+myx),
			// (int)(Math.cos(Math.toRadians((smThread.cnt%72)*5))*16+myy),
			// (main.smThread.cnt/6%7)*64,0, 64,64, 4);//실드 라이프가 3 이상
			...
			// (int)(Math.sin(Math.toRadians((smThread.cnt%72)*5))*16+myx),
			// (int)(Math.cos(Math.toRadians((smThread.cnt%72)*5))*16+myy),
			// (main.smThread.cnt/6%7)*64,0, 64,64, 4);//실드 라이프가 1, 2면 점멸
		}
	...
	public void process() {
		...
		case 2:
			if (imgIndex == 1) {
				if (smThread.cnt % 8 == 0 && isShoot) {
					isShoot = false;
					Bullet shoot = new Bullet(x + 5000, y + 4000, power - 1, playerNum, 270, 10, power);
					// gamePanel.add(shoot);
					bullets.add(shoot);
				}
			}
		...
		if (x < 0)
			x = 0;
		if (x > SMFrame.SCREEN_WIDTH * 100 - 7000)
			x = SMFrame.SCREEN_WIDTH * 100 - 7000;
		if (y < 3000)
			y = 3000;
		if (y > SMFrame.SCREEN_HEIGHT * 100 - 10000)
			y = SMFrame.SCREEN_HEIGHT * 100 - 10000;
	...
	public void setImg() {
		...
		for (int i = 0; i < 9; i++) {
			// if(i < 10)
			character[i] = Toolkit.getDefaultToolkit().getImage("res/character/c" + playerNum + "_0" + i + ".png");
